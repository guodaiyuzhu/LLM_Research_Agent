...
from data.datasets import IntentSlotDataset
from model.models import JointBert
from utils.tools import save_module
...
def train(args):
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in model.named_parameters() if not any(nd in n for nd in parameter_names_no_decay)], 'weight_decay': args.weight_decay},
        {'params': [p for n, p in model.named_parameters() if any(nd in n for nd in parameter_names_no_decay)], 'weight_decay': 0.0}
    ]
    ...
    for epoch in range(args.train_epochs):
        ...
        for batch in dataloader:
            ...
            outputs = model(input_ids=torch.tensor(input_ids).long().to(device), intent_labels=torch.tensor(intent_labels).long().to(device), 
...
if __name__=='__main__':
    ...
    # Data parameters
    parser.add_argument("--train_data_path", type=str, default='data/sample/expanded_train.json', help='training data path')
    parser.add_argument("--test_data_path", type=str, default='data/sample/expanded_test.json', help='testing data path')
    parser.add_argument("--slot_label_path", type=str, default='data/sample/slot_labels.txt', help='Slot Label path')
    parser.add_argument("--intent_label_path", type=str, default='data/sample/intent_labels.txt', help='intent label path')

    # Training parameters
    parser.add_argument("--save_dir", type=str, default='bert-base-chinese', help='directory to save the model')
    ...
    # parser.add_argument("--saving_steps", type=int, default=100, help='parameter update step number to save model')
    ...
    parser.add_argument("--adam_epsilon", type=float, default=1e-12, help='epsilon for Adam optimizer')
