...
from LLM_ChatIntent_Pro.data.labeldict import LabelDict
...
class JointIntentSlotDetector():
    ...
    def _extract_slots_from_labels(self, input_ids, slot_labels, mask=None):
        ...
        if mask is None:
            mask = [1 for _ in id_seq for id_seq in range(len(input_ids))]
    ...
    def detect(self, text, str_lower_case=True):
        """
        text : list of string, each string is an utterance from user
        """
        list_input = True
        ...
        outputs = [
            {'text': text[i], 'intent': intent_labels[i], 'slots': slot_values[i], 'intent_probs': str(intent_probs[i])}
...
if __name__ == "__main__":
    ...
    model_path = 'bert-base-chinese/model/model_step100'
    data_path = 'data/sample/test.json'
    tokenizer_path = 'bert-base-chinese'
    intent_path = 'data/sample/intent_labels.txt'
    slot_path = 'data/sample/slot_labels.txt'
    ...
    print(detector.detect("你好，我想知道我的航班什么时候起飞"))
    print(detector.detect("帮我预定一张飞往上海的机票"))
    print(detector.detect("天气预报显示今天下午会下雨"))
